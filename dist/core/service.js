...
export class ConverseService extends EventEmitter {
    config;
    ownerId;
    vad;
    recorder;
    recorderCarry = Buffer.alloc(0);
    recentEntries = [];
    activeTts;
    speakGeneration = 0;
    lastSpokenHash;
    ...
    async speak(text, ownerId) {
        ...
        for (const chunk of splitSpeechChunks(source)) {
            ...
            if (chunk.pauseSeconds > 0)
                await sleep(chunk.pauseSeconds * 1000);
        }
        return true;
    }
    stopSpeaking() {
        this.speakGeneration += 1;
        if (this.activeTts && !this.activeTts.killed)
            this.activeTts.kill("SIGTERM");
        this.activeTts = undefined;
    }
    onRecorderData(chunk) {
        ...
        while (this.recorderCarry.length >= frameBytes) {
            ...
            for (const emission of this.vad.pushFrame(frame)) {
                void this.handleVadEmission(emission).catch((error) => {
                    this.emit("error", error instanceof Error ? error : new Error(String(error)));
                });
            }
        }
    }
    async handleVadEmission(emission) {
        if (emission.type === "barge-in") {
            this.stopSpeaking();
            this.emit("barge-in");
            return;
        }
        if (emission.type === "speech-start")
            return;
    ...
    async transcribe(audio) {
        ...
        if (this.config.whisperPrompt)
            form.set("prompt", this.config.whisperPrompt);
        ...
        if (!response.ok)
            throw new Error(`Whisper request failed: ${response.status}`);
