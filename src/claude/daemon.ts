...
for (const arg of process.argv.slice(2)) {
  ...
  if (key.startsWith("--") && value) argv.set(key.slice(2), value);
}
...
const finalSubscribers = new Set<http.ServerResponse>();
...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.method === "POST" && url.pathname === "/v1/speak") {
      ...
      const payload = JSON.parse(body) as { owner_id?: string; text?: string };
    ...
    if (request.method === "POST" && url.pathname === "/v1/shutdown") {
      const body = await readBody(request);
      const payload = body ? (JSON.parse(body) as { owner_id?: string }) : {};
...
const shutdown = async (code: number): Promise<void> => {
  for (const subscriber of finalSubscribers) subscriber.end();
