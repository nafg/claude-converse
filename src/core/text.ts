...
const splitSentences = (text: string): string[] => {
  ...
  for (const word of words) {
    ...
    if (ABBREVS.test(joined)) continue;
...
export const splitSpeechChunks = (text: string): SpeechChunk[] => {
  if (!text.trim()) return [];
  ...
  paragraphs.forEach((paragraph, paragraphIndex) => {
    ...
    if (isList) {
      for (const line of lines) {
        ...
        if (clean) chunks.push({ text: clean, pauseSeconds: SENTENCE_PAUSE });
      }
    } else {
      ...
      for (const sentence of splitSentences(fullText)) {
        ...
        if (trimmed) chunks.push({ text: trimmed, pauseSeconds: SENTENCE_PAUSE });
      }
    }
  ...
  if (chunks.length > 0) chunks[chunks.length - 1]!.pauseSeconds = 0;
  return chunks;
};

export const speakableText = (text: string): string | null => {
  const stripped = stripEchoPrefix(text).trim();
  return stripped ? stripped : null;
};

export const hashText = (text: string): string => crypto.createHash("md5").update(text).digest("hex");
